...
menu_options = ["📊 삼성바이오 실적 분석", "🏢 글로벌 빅파마 실적 및 시사점", "🔬 AI 가상 연구소 동향", "🏫 국제중학교 입시설계", "₿ 가상화폐 매매 현황"]
if dev_mode:
    menu_options.extend(["📂 경력 모니터링"])
...
def show_crypto_page():
    st.title("₿ 가상화폐 매매 현황")
    st.success("🔓 BTC/ETH 자동 매매 최신 내역입니다. (단위: 1시간 스캐닝 중 매수 체결 건만 서버에서 자동 업로드됨)")
